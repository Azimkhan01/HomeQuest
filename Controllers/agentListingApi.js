...
const agentListingApi = async (req, res) => {
  try {
    let { id } = req.params;
...
// console.log(offset)
    ...
    if (isNaN(offset) || offset < 0) {
      data = await listing.find({role:"agent"}); // Return all listings
    // console.log('data is loaded')
    } else {
      // Fetch listings with pagination
      data = await listing.find({role:'agent'}).skip(offset).limit(10);
    }
...
const getAgentPropertyDetails = async (req, res) => {
    try {
...
module.exports = {agentListingApi};
